...
def load_data_and_labels(fname, limit=LIMIT, split=0.8):
    ...
    # for k, v in topic_wise_pid.items():
    #     print(k, len(v))

    # for k, v in topic_wise_source.items():
    #     print(k, v)
...
def deepnn(x):
  ...
  # x_image = tf.reshape(x, [None, 28, LIMIT, 1])

  # First convolutional layer - maps one grayscale image to 32 feature maps.
  W_conv1 = weight_variable([28, 25, 1, 128])
  ...
  final_dimen = 128
  W_fc1 = weight_variable([final_dimen, 128])
  b_fc1 = bias_variable([128])

  h_pool2_flat = tf.reshape(h_pool1, [-1, final_dimen])
  ...
  W_fc2 = weight_variable([128, len(CONCERNED_TAGS)])
...
def max_pool_2x2(x):
  ...
  return tf.nn.max_pool(x, ksize=[1, 1, 776, 1],
                        strides=[1, 1, 776, 1], padding='SAME')
...
def transform(strings):
    ...
    for string in strings:
        vec = vectorizer.alpha_ascii_vectorizer_sparse(string, LIMIT)
...
def main(_):
  ...
  # Create the model
  x = tf.placeholder(tf.float32, [None, 28, LIMIT, 1])
  ...
  with tf.Session() as sess:
    ...
    for idx, batch in enumerate(batch_iter(list(zip(X_train, Y_train)))):
      ...
      if idx % 100 == 0:
        ...
        saver.save(sess, './model.simplest')
